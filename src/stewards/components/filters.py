...
from stewards.monitors.transforms import RowLike, filter_options
...
def render_filters(monitor: Monitor, rows: Sequence[RowLike]) -> FilterState:
    ...
    for column, spec in zip(columns[1:], monitor.filters, strict=False):
        options = [ALL, *filter_options(monitor, rows, spec.field)]
    ...
    if monitor.has_threshold_filter:
        past_only = columns[-1].toggle(
            ...
            help=monitor.threshold_help
            or f"Show only incidents open {monitor.threshold_days} days or more.",
        )
